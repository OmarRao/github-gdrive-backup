    ...
    if (INCLUDE.includes('code')) {
      ...
      if (!encrypted) {
        ...
        if (!driveSize || driveSize !== localSize) {
          logger.error(`Verification failed for ${logger.sanitize(name)}: local size ${localSize} bytes, Drive size ${driveSize || 0} bytes`);
          throw new Error(`Upload verification failed: size mismatch for ${baseName}`);
        }
        logger.info(`Verified ${logger.sanitize(baseName)}: ${localSize} bytes matches Drive`);
      } else {
        logger.info(`Uploaded encrypted ${logger.sanitize(uploadFileName)} (original size: ${localSize} bytes)`);
      }
    ...
    if (INCLUDE.includes('wiki')) {
      ...
      if (wikiDir) {
        ...
        if (!wikiDriveSize || wikiDriveSize !== wikiLocalSize) {
          logger.error(`Verification failed for ${logger.sanitize(name)} wiki: local size ${wikiLocalSize} bytes, Drive size ${wikiDriveSize || 0} bytes`);
          throw new Error(`Upload verification failed: size mismatch for ${name}-wiki.zip`);
        }
        logger.info(`Verified ${logger.sanitize(name)}-wiki.zip: ${wikiLocalSize} bytes matches Drive`);
