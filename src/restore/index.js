...
async function deliverGit(provider, repoDir, targetOwner, repoName) {
  ...
  let mismatches;
  const dest = `${encodeURIComponent(targetOwner)}/${encodeURIComponent(repoName)}`;
  try {
    ...
    logger.warn(`Could not verify remote refs for ${dest}: ${encodeURIComponent(e.message)}`);
...
async function restoreRepo(provider, drive, repoFiles, owner, options = {}, manifestHashes = null, ctx = null) {
  ...
  if (options.recreateIssues && meta.issues?.length && typeof provider.restoreIssues === 'function') {
    ...
    logger.info(`Re-created ${n} issue(s) on ${encodeURIComponent(targetOwner)}/${encodeURIComponent(repoName)}`);
  }

  logger.info(`✓ Restored ${encodeURIComponent(meta.repo)} → ${provider.id}:${encodeURIComponent(targetOwner)}/${encodeURIComponent(repoName)}`);
  return {
    original: meta.repo,
...
async function runRestore(options = {}) {
  ...
  if (process.env.DRY_RUN === 'true') {
    ...
    logger.info(`Session: ${encodeURIComponent(sessionName)} (id: ${encodeURIComponent(sessionId)})`);
